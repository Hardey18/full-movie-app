...
function RecentlyAdded({ simplified }: any) {
    const [realRecentlyAdded, setRealRecentlyAdded] = useState([]);

    const { data: getRecentlyAdded } = useGetRecentlyAddedQuery({
    ...
    useEffect(() => {
        arrayOfMovieImages = getRecentlyAdded?.movie_results?.map(
            (result: any) => {
                return API.get(
    ...
    }, [getRecentlyAdded, arrayOfMovieImages]);

    async function recentlyAddedFunction(param: any) {
        let resolvedDetails: any = await Promise.all(param);
        setRealRecentlyAdded(resolvedDetails);
    }
    return (
        <>
            {!simplified && (
                <div className="search-crypto">
                    <Input placeholder="Search Recently Added" />
                </div>
            )}
            {!simplified && (
                <Row gutter={[32, 32]} className="crypto-card-container">
                    {realRecentlyAdded?.map((result: any) => (
                        <Col
                            xs={24}
                            sm={12}
                            xl={6}
